...
nvm などで Node を入れている場合は、ダブルクリックでは `node` が見つからないので、ターミナルから `bash start.sh` で起動してください。
...
| `STATE_FILE` | 任意 | 受信済み通知・サーバー名・ペアリング済みデバイス・題名ごとの発報記録の置き場（既定 `state.json`。空にすると既定値）。既定以外の場所にすると `.gitignore` の対象外になるので注意 |
